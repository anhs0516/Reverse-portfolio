...
이로써 RSA 암호화의 공개키인 N, e 값을 찾게되었습니다.

비밀키인 N, d 중 d 값을 찾는 공식은 아래와 같습니다.

RSA 복호화의 핵심 계산으로 변환된 숫자 s를 복호화 키인 d로 거듭제곱 후 n으로 나눈 나머지 값을 구합니다

<img width="124" height="40" alt="image" src="https://github.com/user-attachments/assets/3b84c89d-865e-46fa-93db-b1d01e0194cd" />

이 내용을 파이썬으로 코드화시키면 아래와 같습니다.
...
1. 왜 8바이트씩 잘랐는가?
 - out.bin 파일 내 88byte가 존재하고 00 00 00 00 4바이트가 지속적으로 붙어있는 것을 보았을 때, 나머지 4바이트를 0으로 패딩하여 8바이트로 만든것으로 판단됨
 - 결과적으로 88byte를 8byte로 나눠 11개의 글자가 확인됨

2. 왜 4바이트씩 읽어들이는가?
 - n의 값 즉, 44271010253을 비트로 확인해보면 32비트인것으로 확인됨 고로 복호화 결과는 4바이트(32비트)를 넘을 수 없음  


## 느낀점

우선 암호화 공식을 보자마자 당황했다. 처음에 공식을 봤어도 RSA를 몰랐기도 했지만 이를 직접 해석해서 풀면 될 것이라 생각했는데 아무리 해도 잘 안풀렸다.. 실력이 부족해서겠지

RSA를 몰라도 역으로 혼자 풀 수 있는 정도가 안된다.... 아직 많이 부족하고 더 많은 암호화 문제를 풀어봐야겠다.

문제를 풀면서 두 가지 목표가 생겼다.

1. 문제들을 보면 어떤 암호화로 구성되었는지 바로 알기
2. 공식인지 몰라도 혼자 역으로 풀 수 있을 정도의 실력자가 되기
