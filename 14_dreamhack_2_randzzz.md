...
다음 if문도 걸리게 하여야만 전체의 flag를 얻을 수 있을 것으로 보입니다.
...
해당 if문들을 지나가기 위해 각 if문 실행 부분에 breakpoint를 걸어 입력되어 있는 숫자를 수정하기 

처음 if문에 5, 두 번째 if문에 3으로 변경하여 flag 획득



### 느낀점

처음 시작할 때 IDA를 통한 그래프 분석, 코드 분석 부분이 아직 많이 부족함

그래프 분석, 코드 분석을 보고 어떤식으로 흘러가겠구나를 빠르고 정확하게 느낀다면 문제 푸는 시간을 많이 단축시킬 수 있을 것으로 보임

GDB를 통한 분석은 지속적으로 사용해주지 않으면 잊어버리기 때문에 지속적으로 문제 푸는게 중요하다고 느꼈음
